...
const downloadApp = async (platform: 'android' | 'ios') => {
  const fileName = platform === 'android' ? 'koisend.apk' : 'koisend.ipa';

  try {
    // Generate a signed URL valid for 5 minutes
    const { data, error } = await supabase.storage
      .from('apps') // Name of your Supabase bucket
...
<template>
  <ion-page>
    <ion-content>
      ...
      <section class="hero-section">
        <div class="hero-content">
          ...
          <p class="hero-description">
            Envía productos y recarga desde cualquier lugar del mundo
          </p>
      ...
      <!-- Reemplaza la sección "Top-up Service Banner" actual con este código: -->

      <!-- Services Section (Top-up & Remittances) -->
        ...
        <div class="services-grid">
          ...
          <ion-card class="service-card">
            <ion-card-content class="service-content">
              ...
              <ion-button expand="block" size="large" class="service-btn" @click="openTopUpWhatsApp">
          ...
          <ion-card class="service-card">
            <ion-card-content class="service-content">
              ...
              <ion-button expand="block" size="large" class="service-btn" @click="openRemittanceWhatsApp">
                <ion-icon slot="start" :icon="logoWhatsapp" />
                Enviar Remesa
              </ion-button>
            </ion-card-content>
      ...
      <section class="download-section">
        <div class="download-container">
          <div class="download-text">
            <h2 class="download-title">Lleva KOISEND en tu bolsillo</h2>
            <p>Descarga nuestra app nativa para una experiencia más rápida y sin gastar datos extras.</p>
          </div>

          <div class="download-actions">
            <ion-button expand="block" class="download-btn android" @click="downloadApp('android')">
              <ion-icon slot="start" :icon="logoAndroid" />
              <div class="btn-label">
            ...
            <ion-button expand="block" class="download-btn ios" @click="downloadApp('ios')">
              <ion-icon slot="start" :icon="logoApple" />
              <div class="btn-label">
                <span class="btn-small">Instalar en</span>
                <span class="btn-large">iOS (Jailbreak)</span>
              </div>
            </ion-button>
          </div>

          <p class="download-note">
            * Para iOS requiere tener el dispositivo liberado (Jailbreak).
          </p>
        </div>
      </section>
    </ion-content>
...
.download-container {
  ...
  background: var(--koi-neutral-100);
  padding: var(--koi-space-6);
  border-radius: var(--koi-radius-2xl);
  text-align: center;
  border: 1px solid var(--koi-neutral-200);
}
